...
class LayerNorm(nn.Module):
    def __init__(self, ndim, bias):
        ...
        self.epsilon = 1e-5
        # dict to store intermediate values <aman>
        self.dict = {}

    def forward(self, input):
        layer_norm = F.layer_norm(
            input, self.weight.shape, self.weight, self.bias, self.epsilon)

        # Compute mean and variance
        input_mean = input.mean(dim=-1, keepdim=True)
        input_var = input.var(dim=-1, unbiased=False, keepdim=True)
        input_normalized = (input - input_mean) / \
            torch.sqrt(input_var + self.epsilon)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # get weights
        q_weights, k_weights, v_weights = self.c_attn.weight.split(
            self.n_embd, dim=0)
        # split into multihead weights
        q_weights = q_weights.view(
            self.n_head, C // self.n_head, C)  # (nh, hs, C)
        k_weights = k_weights.view(
            self.n_head, C // self.n_head, C)  # (nh, hs, C)
        ...
        for i in range(self.n_head):
            self.dict[f"head_{i}"] = {}
            q_transposed, k_transposed, v_transposed = q.transpose(
                -2, -1), k.transpose(-2, -1), v.transpose(-2, -1)

            # Store attention-related outputs for each head
            self.dict[f"head_{i}"]["attn"] = attn[:,
                                                  i]                  # Attention scores
            ...
            self.dict[f"head_{i}"]["k"] = k[:, i]
            self.dict[f"head_{i}"]["v"] = v[:, i]
        ...
        # store final attention output <aman>
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # store residual connection result <aman>
        x = x + self.attn(ln_1_output)
        self.dict["res_1"] = x
